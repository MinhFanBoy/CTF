...
hmm. Mình có sử dụng wu trên mạng để viết lại.

Mình sử dụng brute force để tìm ra hai đoạn đầu vào khác nhau nhưng có chung kết quả. Vì trong Sbox có khá nhiều giá trị bị lặp lại nên nó không thực sự là hoán vị khiến việc brute trở nên khả thì vì có đầu vào là 2 ^ 16 rất dễ để tìm ra.
...
def brute_force(data: bytes) -> bytes:
...
def main() -> None:
    ...
    while True:
        ...
        test: bytes = bytes(os.urandom(4))
        result: bytes = brute_force(test)

        if result in store.keys() and test not in store.values():

            hash_1, hash_2 = test, store[result]
            break

        store[result] = test
    
    print(f"Found hash_1: {hash_1}")
    print(f"Found hash_2: {hash_2}")
    print(f"Found result: {result}")
    
    s = connect("socket.cryptohack.org", 13395)
    print(s.recv().decode())
    s.sendline(dumps({"a": hash_1.hex(), "b": hash_2.hex()}).encode())
    print(loads(s.recv()))
