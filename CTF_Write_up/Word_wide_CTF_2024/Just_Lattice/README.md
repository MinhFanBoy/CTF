...
+ Hàm chính trong bài này chủ yếu làm hàm `enc` các hàm còn lại không quan trọng nên ta sẽ bỏ qua và đi đến hàm chính ngay:
...
+ Z[i] = [1, 0, 0, 0]
+ np.floor(q/2) = 63
+ M[i] là bit của flag nên có thể bằng 0 hoặc 1

C = P * r + (0, 1) * [63, 0, 0, 0]

Khi này mình ghép ma trận P và vector([63, 0, 0, 0]) lại thành 1 ma trân tạm gọi là P' khi đó C là tích của vector P' với r. Do r' = r + [0] hoặc r + [1] nên C = P' * r', r' là ma trận gồm toàn các số 0, 1 nên mình sử dụng solve_linear để tìm lại r' và từ đó có thể lấy lại được các bit của flag.
...
for k in C:
    ...
    M = block_matrix([
        [P],
        [column_matrix([63, 0, 0, 0]).T]])
    k = list(k)
    ub = [1] * 16
    lb = [0] * 16

    for i in solve_bounded_mod_gen(M.T, k, lb, ub, q):
        print(i)
        if i[-1] == 0:
            flag += str(0)
        else:
