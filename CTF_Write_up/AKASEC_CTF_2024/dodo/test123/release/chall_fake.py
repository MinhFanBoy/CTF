...
class myencryption():
    ...
    def generate_key(self):
        ...
        return bytes.fromhex(hex(key)[2:].zfill(32))

    def get_recognizer(self):
        self.recognizer = {"id": int(time.time()) * 0xff, "username": self.username.encode(), "is_admin": False, "key": self.keys[self.key_counter].hex()}
...
def main():
    ...
    while True:
        ...
        if choice == 1:
            ...
            ans = cipher.generate_key().hex()
            if your_bet == ans:
                print("Congratulation ! Here is the flag: AKASEC{nn_hh}")
            else:
                print("I am sorry, but you are not the admin.", int(ans, 16))
                exit()
        elif choice == 3:
            exit()
        else:
